<template>
    <div class="container1">
        <div class="map">
            <!-- <button style='position:absolute;left:55px;top:15px;z-index:999;background-color:transparent;' @click='change'> -->
            <!-- <span>全屏</span><i class="dmap dmap-screen" /> -->
            <!-- </button> -->
            <dmap
         ...
         <div class="table">
             ...
             <div class="show" ref="show" >
               <div class="columns">
                  <div v-for="item in columns" :key="item.label" class="label" :style="item.style">{{item.label}}</div>                    
               </div>   
               <div class="content" ref="content" v-if="testData.length!==0" :style="tableHeight" >
                   <div class="content_index" v-for="(item,index) in testData" :key="index" @dblclick="showPoint(item)" @click="showPoint1(item)" >
                     <div v-for="item1 in columns" :key="item1.field" :style="item1.style">{{item[item1.field] | fmt(item1.field,index)}}
...
export default {
      ...
      columns: {
          ...
          default: () => {
              return [  
                ...
                { label: "地址", field: 'address', style: { textAlign: 'center', width: '120px' } },
                { label: "物资点", field: 'stnm', style: { textAlign: 'center', width: '100px' } },
                { label: "物资类型", field: 'type_name', style: { textAlign: 'center',width: '80px'} },
            ]
          }
      },
     testData:{
    ...
    methods:{
        getData() {
            ...
            //   })
            this.$Axios
              .get('http://192.168.118.226:9831/WebApi/api/v2/basic/data?key=s_area_by_material',{
